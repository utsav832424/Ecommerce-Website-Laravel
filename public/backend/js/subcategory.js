...
$(function() {
    ...
    $('#category_form').on('submit', function(e) {
        ...
        if (categoryname == "") {
            ...
            toastr.error('Please Enter Sub Category Slug')
        } else {
            ...
            if (editId != 0) {
                action = action.replace("addSubCategory", "updateSubCategory");
    ...
    $('#categoriesTable tbody').on('click','.dropdown-item.edit', function() {
        $('#staticBackdropLabel').html("Edit Sub Category");
        $('#staticBackdrop').find('.modal-footer').find('.btn-success').text('Save');
        editId = $(this).closest('tr').data('id');
        console.log(editId);
        var categoryId = $(this).closest('tr').children().eq(1).data('categoryid');
        var name = $(this).closest('tr').children().eq(2).text();
        var slug = $(this).closest('tr').children().eq(3).text();
        var active = $(this).closest('tr').children().eq(4).text();
        // console.log("categoryId",categoryId);
        $('#category-name').val(categoryId).trigger('change');
        $('#name').val(name);
        $('#slug').val(slug);
    ...
    $('#AddCategoryBtn').click(function() {
        $('#staticBackdropLabel').html("Add Sub Category");
        ...
        success: function(res) {
            ...
            res.data.forEach((element,index) => {
                html += `<tr data-id="${element.id}">
                    ...
                    <td data-categoryID="${element.category_id}">${element.category_name}</td>
                    <td>${element.name}</td>
                    <td>${element.slug}</td>`;
